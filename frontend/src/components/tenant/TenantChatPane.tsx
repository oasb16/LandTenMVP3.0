...
import { useTenantChat } from "./TenantChatProvider";
...
export default function TenantChatPane({ className, showEscalation, onEscalate }: Props) {
  const {
    client,
    activeChannel,
    loading,
...
function MessageInputWithWebhook({ agentEnabled }: { agentEnabled: boolean }) {
  const {sendMessage, user, activeChannel } = useTenantChat();
