...
export default function Chat({ user }) {
  ...
  const sendPayload = async (payload, { optimistic = true } = {}) => {
    ...
    if (optimistic) {
      setMessages((prev) => [...prev, { ...basePayload, optimistic: true }]);
    }
    try {
      ...
      if (optimistic) {
        setMessages((prev) => prev.filter((msg) => msg.client_id !== clientId));
      }
      throw err;
    }
  };
    ...
    if (msg.type === "card" && msg.payload) {
      return (
        <div style={{ border: "1px solid #1f2937", borderRadius: 8, padding: 8 }}>
          <div style={{ fontWeight: 700 }}>{msg.payload.title}</div>
          <div style={{ marginTop: 4 }}>{msg.payload.body}</div>
        </div>
      );
    }
